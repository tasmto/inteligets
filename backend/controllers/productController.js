...
const getProducts = asyncHandler(async (req, res) => {
  ...
  const pageSize = process.env.DEFAULT_PAGE_SIZE;
  ...
  res.json({ products, page, pages: Math.ceil(count / pageSize) }); // both res.send and re.json converts it to JSON
});
...
const createProductReview = asyncHandler(async (req, res) => {
  ...
  if (product) {
    ...
    if (alreadyReviewed) {
      res.status(400);
      throw new Error('This account has already reviewed this product.');
    }
    const review = {
      name: req.user.name,
